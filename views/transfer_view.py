...
class TransferView(discord.ui.LayoutView):
    ...
    def confirmation_text(self):
        return (
            f"### 🔁 Potwierdzenie przekazania\n\n"
            f"**Od:** {self.sender.mention}\n"
            f"**Dla:** {self.receiver.mention}\n"
            f"**Ilość:** `{self.amount} {self.currency}`\n\n"
            f"Czy na pewno chcesz wykonać tę operację?"
        )
    ...
    def render_confirmation(self):
        ...
        self.add_item(make_container(self.confirmation_text(), discord.Color.gold(), confirm_button, cancel_button))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.sender.id:
            await interaction.response.send_message(
                view=CardView("### ❌ Komunikat\n\nTylko osoba wykonująca operację może używać tych przycisków.", discord.Color.red()),
                ephemeral=True
            )
            return False
        return True
    ...
    async def confirm_transfer(self, interaction: discord.Interaction):
        ...
        if sender_data["balance"] < self.amount:
            text = (
                f"### ❌ Operacja anulowana\n\n"
                f"Nie masz już wystarczająco {self.currency}.\n"
                f"Stan konta: `{sender_data['balance']} {self.currency}`"
            )
            return await interaction.response.edit_message(view=CardView(text, discord.Color.red()))
    ...
    async def cancel_transfer(self, interaction: discord.Interaction):
        await interaction.response.edit_message(view=CardView("### ❌ Operacja została anulowana", discord.Color.red()))
